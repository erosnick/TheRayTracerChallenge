...
void render(unsigned int texture1, unsigned int texture2, Shader& ourShader, unsigned int VAO) {
    // render
         // ------
...
int main() {
    ...
    unsigned char* data = stbi_load(FileSystem::getPath("resources/textures/Test.png").c_str(), &width, &height, &nrChannels, 0);
    if (data) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        render(texture1, texture2, ourShader, VAO);
